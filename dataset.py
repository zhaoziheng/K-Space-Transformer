...
class TestDataSet(Dataset):
  """
  Different from validation, TestDataSet dosen't require groundtruth downsampled low-resolution data 
  """
  def __init__(self, dataPath, maskPath):
    self.k_gt, self.i_gt, self.mask, \
    self.sampled_num, self.unsampled_num, \
    self.LR_pos, self.LR_pos_norm = self.read_data(dataPath, maskPath)  # [num_of_sample, h, w, 2]
  ...
  def read_data(self, dataPath, maskPath, LR_resolution=[64, 64]):
    k_data = np.load(dataPath)  # [num_of_sample, H, W, 2]

    self.HR_grid = build_grid([k_data.shape[1], k_data.shape[2]])  # h, w
    self.LR_grid = build_grid([LR_resolution[0], LR_resolution[1]])  # lh, lw

    k_data = torch.from_numpy(k_data)
    i_data = ifft2c_new(k_data, needShift=True)

    LR_pos_seq = [[], []]
    for i in range(LR_resolution[0]):
      for j in range(LR_resolution[1]):
        LR_pos_seq[0].append(i)
        LR_pos_seq[1].append(j)
    LR_pos_seq_norm = self.LR_grid[LR_pos_seq]  # [num of unsampled points, 2]
    LR_pos_seq = np.array(LR_pos_seq).transpose(1, 0)  # [num of unsampled points, 2]
    ...
    return k_data, i_data, mask, sampled_num, unsampled_num, LR_pos_seq, LR_pos_seq_norm
